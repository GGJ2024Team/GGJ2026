...
const ENEMY_SCENES: Dictionary = {
    "FLYING_CREATURE": preload("res://Characters/Enemies/FlyingCreature/FlyingCreature.tscn"),
...
func _close_entrance() -> void:
    for entry_position in entrance.get_children():
        tilemap.set_cellv(tilemap.world_to_map(entry_position.position), 14)
        tilemap.set_cellv(tilemap.world_to_map(entry_position.position) + Vector2.DOWN, 10)
        
        
func _spawn_enemies() -> void:
    for enemy_position in enemy_positions_container.get_children():
        var enemy: KinematicBody2D = ENEMY_SCENES.values()[randi()%ENEMY_SCENES.size()].instance()
