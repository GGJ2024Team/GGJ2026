...
func interpolate_pos(initial_pos: Vector2, final_pos: Vector2) -> void:
    print("interpolate_pos")
    var __ = tween.interpolate_property(self, 
        "position", initial_pos, final_pos, 0.8, Tween.TRANS_QUART, Tween.EASE_OUT)
    assert(__)
    __ = tween.start()
    assert(__)
...
func _on_PlayerDetector_body_entered(body: KinematicBody2D):
    if body != null:
        ...
        print("collision with something not body")
